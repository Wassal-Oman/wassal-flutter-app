...
class _CustomerRegisterState extends State<CustomerRegister> {
  ...
  void signUp() {
    ...
    if (password == confirmPassword) {
      ...
      signingUp().then((FirebaseUser user) {
        ...
        if(user != null) {
          ...
          user.sendEmailVerification().then((val) {
            ...
            // Scaffold.of(context).showSnackBar(SnackBar(content: Text('Verification mail has been sent!')));
          }).catchError((AuthException e) {
            print(e.message);
            // Scaffold.of(context).showSnackBar(SnackBar(content: Text('Cannot send verification mail!')));
          });
          ...
          addUserToDatabase(user.uid, user.email, this.name, this.phone).then((val) {
            ...
            print(err);
            // showDailog(context, 'Failure', 'Cannot register user!');
          });
        }
      }).catchError((AuthException e) {
        print(e.message);
      ...
      // Scaffold.of(context).showSnackBar(SnackBar(content: Text('Passwords do not match!')));
    }
